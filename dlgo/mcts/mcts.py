...
class MCTSAgent(agent.Agent):
    ...
    def select_move(self, game_state):
        ...
        nbest = 20
        print( 'best %d moves:' % nbest)
        scored_moves.sort(key=lambda x: x[0], reverse=True)
        for s, m, n in scored_moves[:nbest]:
            print('%s - %.3f (%d)' % (m, s, n))
        ...
        print('%s: Select move %s with win pct %.3f' % (game_state.next_player, best_move, best_pct))
        return best_move
