...
# /********************************************************************
# Filename: model_large_conv_pool.py
# Author: AHN
# Creation Date: Apr 2019
# **********************************************************************/
#
# large.py from dlgo adapted for score estimation.
...
class Model:
    ...
    def build_model(self):
        ...
        x = kl.Conv2D( 64, (7,7), activation='relu', padding='same', name='one')(inputs)
        x = kl.BatchNormalization(axis=-1)(x)
        x = kl.Conv2D( 64, (5,5), activation='relu', padding='same', name='two')(x)
        x = kl.BatchNormalization(axis=-1)(x)
        x = kl.Conv2D( 64, (5,5), activation='relu', padding='same', name='three')(x)
        x = kl.BatchNormalization(axis=-1)(x)
        x = kl.Conv2D( 48, (5,5), activation='relu', padding='same', name='four')(x)
        x = kl.BatchNormalization(axis=-1)(x)
        x = kl.Conv2D( 48, (5,5), activation='relu', padding='same', name='five')(x)
        x = kl.BatchNormalization(axis=-1)(x)
        x = kl.Conv2D( 32, (5,5), activation='relu', padding='same', name='six')(x)
        x = kl.BatchNormalization(axis=-1)(x)
        x = kl.Conv2D( 32, (5,5), activation='relu', padding='same', name='seven')(x)
        x = kl.BatchNormalization(axis=-1)(x)
